...
- Pin every external GitHub Action to a full release commit SHA with a version comment. When updating, verify the release tag resolves to that commit and the commit contains `action.yml`. A release such as `v10.2.0` does not guarantee a `v10` alias exists.
